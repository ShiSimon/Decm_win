...
static inline void crypt(AVAES *a, int s, uint8_t *sbox, uint32_t multbl[4][256])
{
    int r;
...
void av_aes_crypt(AVAES *a, uint8_t *dst, uint8_t *src, int count, uint8_t *iv, int decrypt)
{
    while(count--)
    {
        ...
        if(decrypt) 
        {
            crypt(a, 0, inv_sbox, (uint32_t(*)[256])dec_multbl);
            if(iv)
            {
                addkey((uint64_t*)a->state[0], (uint64_t*)a->state[0], (uint64_t*)iv);
                memcpy(iv, src, 16);
            }
        ...
        {
            if(iv) addkey((uint64_t*)a->state[1], (uint64_t*)a->state[1], (uint64_t*)iv);
            crypt(a, 2, sbox, (uint32_t(*)[256])enc_multbl);
            addkey((uint64_t*)dst, (uint64_t*)a->state[0], (uint64_t*)a->round_key[0]);
...
int av_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt) 
{
    ...
    uint8_t  log8[256];
    uint8_t alog8[512];
    int tmp=sizeof(enc_multbl)/sizeof(enc_multbl[0][0])-1;
...
//    if(!enc_multbl[0][sizeof(enc_multbl)/sizeof(enc_multbl[0][0])-1])
    if(!enc_multbl[0][tmp])
    {
        ...
        init_multbl2((uint8_t *)dec_multbl[0], (int[4]){0xe, 0x9, 0xd, 0xb}, log8, alog8, inv_sbox);
        init_multbl2((uint8_t *)enc_multbl[0], (int[4]){0x2, 0x1, 0x1, 0x3}, log8, alog8, sbox);
    }
    ...
    if(decrypt)
    {
        for(i=1; i<rounds; i++)
        {
            ...
            mix((uint8_t(*)[4][4])tmp, dec_multbl, 1, 3);
